...
DROP TABLE IF EXISTS public.ro_locations CASCADE;
...
-- Create RO LOCATION UPDATES table
CREATE TABLE public.ro_locations (
  id UUID PRIMARY KEY DEFAULT extensions.gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES public.users(id) ON DELETE CASCADE,
...
CREATE INDEX ro_locations_user_date_idx ON public.ro_locations (user_id, work_date, created_at DESC);
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND schemaname = 'public' AND tablename = 'ro_locations') THEN
